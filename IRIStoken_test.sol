...
contract IrisToken is MintableToken {
  // Coin Properties
...
contract IrisTokenSale is Ownable, Pausable {
  ...
  uint256 public rate;
  uint256 public presale_rate; // how many token units a buyer gets per wei
  uint256 public publicsale_rate; // how many token units a buyer gets per wei
...
// *************************************
    ...
    multiSig = 0xf651e2409120f1FbB0e47812d759e883b5B68A60;
...
//**************************************    
    ...
    oneCoin = 10 ** decimals;
    maxTokens = 200 * (10**6) * oneCoin;  // max number of tokens what we will create
    tokensForSale = 118 * (10**6) * oneCoin; // max number of tokens what we want to sell now
    tokensForPreSale = 55 * (10**6) * oneCoin; // max number of tokens what we want to sell in presale
    presale_rate = 4000;
    publicsale_rate = 2500;
    minContribution = 0.001 ether;
    maxContribution = 20000 ether;
}
/**
  * @dev Calculates the rate in presale and publicsale
    */
  function setTier(uint at) internal {
    
    if (at <= presaleEnd) {
      rate = presale_rate;
    } else 
      rate = publicsale_rate;
    }   
  ...
  */
  ...
  /**
  * @dev set a new presale_rate BE CAREFULL!:
  */
  function setPresaleRate(uint newRate) onlyAdmin public {
    require(!hasEnded());
    require( 0 < newRate && newRate < 8000);
    presale_rate = newRate;
  }
  /**
  * @dev set a new publicsale_rate BE CAREFULL!
  */
  function setPublicsaleRate(uint newRate) onlyAdmin public {
    require(!hasEnded());
    require( 0 < newRate && newRate < 8000);
    publicsale_rate = newRate;
  }
